...
> 状态：环境、AgentLoop、8 条 rollout 已跑通；首次参数更新的 padding fallback 已落库、待 one-step 实测｜日期：2026-07-23

## 摘要

这次工作的目标看似很小：在单张 RTX 4090 48GB 上完成一个 Vanilla GRPO
smoke，批次固定为 `2 prompt × 4 rollout = 8 trajectory`，只更新一次
Qwen3.5-2B 的 LoRA 参数。
...
6. 当前分支已用 Ray worker setup hook 接入 veRL 自带的纯 PyTorch padding fallback，
   但还没有该修复后的 smoke 结果；
7. 因此现有证据仍然是 **尚未完成一次参数更新**，不能提前标记为端到端成功。
...
下一步只需要在 `679e09c` 之后按同样的 one-step 命令重跑，验证 fallback 是否覆盖
实际 Ray worker 的 old-log-prob 路径，以及 optimizer 是否完成一次更新。该次实测
完成前，这仍然是“有针对性的修复”，不是“已验证的训练闭环”。
...
| `679e09c` | padding fallback 已落库 | 尚未重跑 | 尚未重跑 | 尚未重跑 | 等待 one-step 实测 |
...
commit `679e09c` 已为 old-log-prob 安装纯 PyTorch padding fallback，但还没有新的
smoke 日志。因此当前准确状态是：**Agentic rollout 闭环已通过；训练更新修复已落库，
闭环仍待实测。**
