# 从一个能逛商店的模型开始：Shopping Agent 实验札记

> 持续更新中。本文记录已经做过、已经验证过的工作，也记录踩过的坑；没有完成的 SFT 和 GRPO 会明确标为计划，而不是结果。
>
> 最近更新：2026-07-20｜当前阶段：数据冷启动完成，正在建立 Instruct 基线评测
...
目前已经完成服务与工具调用 smoke 验证，完整 200 条 Instruct 基线正在重新启动：旧的 113 条结果采用了错误 parser 且没有输出上限，因此只作为故障记录，不进入实验对比。日常迭代会补充一个独立的 50 题开发集；200 题保留给最终 Base/SFT/GRPO 对比，避免反复调参把测试集“看熟”。
...
后续每完成一个可复现实验，就在本文追加：运行配置、数据/模型版本、固定 benchmark 结果、失败样例和下一步判断。这样这份文档既是项目对外的故事，也是一份不给未来自己挖坑的实验日志。
