...
第一份完整基线已经跑完：Qwen3.5-2B Instruct 完成了全部 200 个 task，但严格成功率为 **0%**，仅 1 条轨迹让环境结束。它并非不会 function calling；真正的瓶颈是零样本策略无法把页面 observation 转化成连续、受约束的点击：159/200 条在连续非法动作后被守卫终止，最常见的是点击不在最新页面中的按钮或商品。完整指标与仍待审计的 29 条 error 见 [实验 01](experiments/01-qwen35-2b-instruct-baseline-2026-07-20.md)。

这是一份很有用的失败基线：它证明 SFT 不只是锦上添花，而是让小模型学会本环境工具语法、页面阅读和行动顺序的必要冷启动。日常迭代会补充一个独立的 50 题开发集；200 题保留给最终 Base/SFT/GRPO 对比，避免反复调参把测试集“看熟”。
