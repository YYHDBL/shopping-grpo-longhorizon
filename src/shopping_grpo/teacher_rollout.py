...
3. 选择前检查点：首次调用 `select_option` 前，必须已在当前商品的标题、详情、规格或价格中逐项证实用户硬约束：型号、产地、材质、颜色/规格、功能和预算。任一硬约束未在当前页面证实，不得购买，也不得选择规格；返回搜索或继续查看，不能先选择规格再补查信息，也不要用相似商品猜测补齐。
4. 购买阶段：检查点通过后才调用 `select_option`。同一规格组只能选择一个值，绝不依次试选同一组多个值；选择后读取最新页面，确认最终价格和 Buy Now。开始选择规格后，系统只会提供 `select_option` 与 `buy_now`：如仍有必要规格，只选择当前页面显示的值；否则在 Buy Now 可见时立即 `buy_now`。`buy_now` 是终止动作，不是跳转到结算页。
    ...
    try:
        ...
        while len(trajectory["steps"]) < int(max_steps):
            assistant = client.complete(
                messages, _tool_schemas_for_state(tool_schemas, selection_started)
            )
...
def _tool_schemas_for_state(tool_schemas, selection_started):
    """选择规格后仅暴露仍合法的购物工具，减少模型状态违规。"""
    if not selection_started:
        return tool_schemas
    allowed_names = {"select_option", "buy_now"}
    return [
        schema
        for schema in tool_schemas
        if (schema.get("function") or {}).get("name") in allowed_names
    ]
