...
4. 选择和购买：只从当前页面选择用户要求的规格；同一规格组只能选择一个值。首次选中规格即结束探索阶段：这表示你已确认当前商品和规格准备购买，不得查看子页、返回搜索、再次搜索或打开其他商品。若仍有未选的必要规格，下一步只能继续 `select_option`；否则，读取选中规格后才显示的真实价格。若符合用户明确预算且 Buy Now 可见，下一步立即 `buy_now`。不要把 `select_option` 当成试探价格或继续比较商品的动作。`buy_now` 是终止动作，不是跳转到结算页。
