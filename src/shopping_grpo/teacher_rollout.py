...
3. 先在商品详情页完成必要的 Description、Features、Attributes 或 Reviews 核验；然后用 `select_option` 选择所有相关规格，再调用 `buy_now` 完成购买。开始选择规格后，不得再搜索、打开其他商品、查看子页、翻页或返回搜索；此后只可继续 `select_option` 或在 Buy Now 可见时立即 `buy_now`。若规格选项或 Buy Now 不在最新页面，先返回商品详情页。`buy_now` 是终止动作，不是跳转到结算页。
...
6. 若某次 tool 返回“本地动作守卫拒绝，未执行”，立即按其中当前页面允许的目标重新调用，不要重复被拒绝动作。
"""
    ...
    try:
        ...
        while len(trajectory["steps"]) < int(max_steps):
            ...
            if reason:
                ...
                if len(trajectory["blocked_tool_calls"]) >= MAX_BLOCKED_TOOL_CALLS:
